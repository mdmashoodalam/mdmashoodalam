# 👨‍💻 Java Full Stack Developer | Md Mashood Alam

🚀 Passionate Computer Science Engineer with strong expertise in building scalable and high-performance web applications using modern technologies.

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=mdmashoodalam&show_icons=true&theme=tokyonight&cache_seconds=60)
---

## 🎓 Education

🎓 B.Tech in Computer Science & Engineering  
📍 RGPV University, Bhopal  

🎓 Diploma in Engineering  
📍 MANUU Polytechnic, Bangalore  

---

## 💡 About Me

- 💻 Java Full Stack Developer with hands-on experience in **Spring Boot, Hibernate, and REST APIs**
- 🌐 Skilled in building responsive UIs using **React.js, Tailwind CSS**
- ⚡ Experience with **MERN Stack (MongoDB, Express, React, Node.js)**
- 🔐 Strong understanding of authentication, API integration & backend architecture
- 📦 Built real-world projects like **E-commerce platforms, Food Ordering Systems & Portfolio Websites**
- 🤝 Open to collaboration & actively exploring **Open Source Contributions**
- 📈 Always learning and adapting to new technologies

---

## 🛠️ Tech Stack

### 🚀 Backend
- Java | Spring Boot | Hibernate | J2EE | REST APIs

### 🎨 Frontend
- React.js | JavaScript | HTML | CSS | Tailwind CSS | Bootstrap

### 🗄️ Database
- MongoDB | MySQL | PostgreSQL

### ⚙️ Tools & Platforms
- Git | GitHub | Maven | Firebase | Linux | Windows

---

## 📌 Featured Projects

🔹 **E-Commerce Web App (MERN Stack)**  
✔️ Dynamic product listing & cart functionality  
✔️ Email integration using EmailJS  

🔹 **Food Ordering System**  
✔️ Real-time order management  
✔️ User-friendly UI & backend integration  

🔹 **Portfolio Website**  
✔️ Modern UI with React  
✔️ Fully responsive design  

---

## 📈 Current Focus

- 🔥 Mastering **Spring Boot Microservices**
- 🚀 Improving **DSA & System Design**
- 🌍 Contributing to **Open Source Projects**

---

## 🤝 Connect With Me

- 💼 Open for **Internships & Full-Time Opportunities**
- 📩 Reach out for collaboration or project discussions

---

⭐ *If you like my work, consider giving a star to my repositories!*
